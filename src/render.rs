...
// Batting view: camera at (0, -cam_back, cam_z), looking toward +y (facing = +1).
// Pitching view: camera at (0, mound_y + cam_back, cam_z'), looking toward -y
//   (facing = -1) — i.e. reversed, standing behind the mound looking at the
//   plate. Turning 180° also mirrors left/right, so the x-axis flips too.
// Projection: sx = cx + x * facing * focal/((y - cam_y) * facing)
//             sy = horizon - (z - cam_z) * focal/((y - cam_y) * facing)
...
pub struct View {
    ...
    pub horizon: f32,
    pub cx:      f32,
    pub facing:  f32,   // +1.0 = looking toward +y (batting), -1.0 = looking toward -y (pitching)
}

impl View {
    /// `reversed`: true for pitching mode (camera behind the mound, looking
    /// back at the plate); false for batting mode (camera behind the plate).
    pub fn new(game: &GameCfg, reversed: bool) -> Self {
        let h = screen_height();
        let w = screen_width();
        if reversed {
            Self {
                cam_y:   MOUND_Y + game.cam_back,
                cam_z:   game.cam_height + 1.2, // mound elevation + eye level
                focal:   game.focal,
                horizon: h * 0.40,
                cx:      w * 0.50,
                facing:  -1.0,
    ...
    pub fn project(&self, p: V3) -> (f32, f32) {
        let depth = (p.y - self.cam_y) * self.facing;
        let s  = self.focal / depth;
        let sx = self.cx + p.x * s * self.facing;
    ...
    pub fn project_s(&self, p: V3) -> (f32, f32, f32) {
        let depth = (p.y - self.cam_y) * self.facing;
        let s  = self.focal / depth;
        (self.cx + p.x * s * self.facing, self.horizon - (p.z - self.cam_z) * s, s)
    }
    ...
    pub fn screen_to_world(&self, mx: f32, my: f32, world_y: f32) -> (f32, f32) {
        let depth = (world_y - self.cam_y) * self.facing;
        let s  = self.focal / depth;
        let wx = (mx - self.cx) / (s * self.facing);
...
// ---------------------------------------------------------------------------
// Fielders — the 7 non-battery defensive positions, shown as simple dots at
// their honest (no-shift) positions. Whichever one is responsible for the
// current play (see outcomes::classify_bip) is drawn separately at its live,
// animated position so the player can watch it converge on the ball.
// ---------------------------------------------------------------------------

pub fn draw_fielders(view: &View, positions: &[(&str, f32, f32)], active: Option<(&str, f32, f32)>) {
    let base_col = Color::from_rgba(230, 210, 60, 235);
    let outline  = Color::from_rgba(40, 40, 20, 200);

    for &(name, fx, fy) in positions {
        if let Some((aname, _, _)) = active {
            if aname == name { continue; } // drawn separately below, at its live position
        }
        let (sx, sy) = view.project(V3::new(fx, fy, 0.0));
        draw_circle(sx, sy, 4.0, base_col);
        draw_circle_lines(sx, sy, 4.0, 1.0, outline);
    }

    if let Some((_, ax, ay)) = active {
        let (sx, sy) = view.project(V3::new(ax, ay, 0.0));
        draw_circle(sx, sy, 5.5, Color::from_rgba(255, 235, 90, 255));
        draw_circle_lines(sx, sy, 8.0, 1.6, Color::from_rgba(255, 255, 255, 190));
    }
}
...
// ---------------------------------------------------------------------------
// Hit FX — a quick, subtle flash and a couple of jagged lightning bolts.
// Brief by design since hits happen often and shouldn't overstay their
// welcome. progress: 0..1 over the effect's lifetime.
// ---------------------------------------------------------------------------

fn draw_lightning_bolt(view_w: f32, view_h: f32, seed: f32, alpha: f32) {
    let x0 = (seed.sin() * 0.5 + 0.5) * view_w;
    let y0 = 0.0f32;
    let x1 = x0 + (seed * 2.7).sin() * 70.0;
    let y1 = view_h * 0.62;
    let segs = 7;
    let mut px = x0;
    let mut py = y0;
    let core = Color::new(0.80, 0.88, 1.0, 0.9 * alpha);
    let glow = Color::new(0.55, 0.70, 1.0, 0.28 * alpha);
    for i in 1..=segs {
        let f = i as f32 / segs as f32;
        let jitter = (seed * 13.0 + i as f32 * 5.7).sin() * 16.0 * (1.0 - (f - 0.5).abs() * 1.3);
        let bx = x0 + (x1 - x0) * f + jitter;
        let by = y0 + (y1 - y0) * f;
        draw_line(px, py, bx, by, 5.0, glow);
        draw_line(px, py, bx, by, 2.0, core);
        px = bx;
        py = by;
    }
}

pub fn draw_hit_fx(progress: f32) {
    let fade = if progress < 0.2 { progress / 0.2 } else { ((1.0 - progress) / 0.8).max(0.0) };
    if fade <= 0.0 { return; }
    ...
    if progress < 0.1 {
        let flash = (1.0 - progress / 0.1).max(0.0);
        draw_rectangle(0.0, 0.0, w, h, Color::new(0.85, 0.92, 1.0, 0.15 * flash));
    }

    draw_lightning_bolt(w, h, 3.7, fade);
    draw_lightning_bolt(w, h, 8.2, fade * 0.7);
}

// ---------------------------------------------------------------------------
// Home run FX — a short fireworks show: a few staggered bursts of colorful
// expanding particles with a bright flash at each burst's core.
// ---------------------------------------------------------------------------

pub fn draw_hr_fx(progress: f32) {
    let w = screen_width();
    let h = screen_height();

    // (start_frac, x_frac, y_frac, hue) — each burst plays over ~42% of the
    // total effect lifetime, staggered so they overlap like a real show.
    let bursts: [(f32, f32, f32, f32); 4] = [
        (0.00, 0.28, 0.30, 0.02),
        (0.16, 0.68, 0.22, 0.33),
        (0.34, 0.45, 0.35, 0.58),
        (0.52, 0.75, 0.40, 0.85),
    ...
    for &(start, xf, yf, hue) in bursts.iter() {
        let local = ((progress - start) / 0.42).clamp(0.0, 1.0);
        if local <= 0.0 || local >= 1.0 { continue; }

        let cx = w * xf;
        let cy = h * yf;
        let n = 20;
        let radius = 95.0 * local;
        let fade = (1.0 - local).powf(1.4);

        for i in 0..n {
            let a = (i as f32 / n as f32) * PI2 + hue * 3.0;
            let px = cx + radius * a.cos();
            let py = cy + radius * a.sin() + local * local * 40.0; // gentle gravity droop
            let (r, g, b) = hsv_to_rgb((hue + i as f32 * 0.01).fract(), 0.85, 1.0);
            draw_circle(px, py, 2.6 * (1.0 - local * 0.4), Color::new(r, g, b, 0.9 * fade));
        }

        if local < 0.18 {
            let flash = 1.0 - local / 0.18;
            draw_circle(cx, cy, 22.0 * flash, Color::new(1.0, 1.0, 0.9, 0.5 * flash));
        }
    }
}
