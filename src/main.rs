...
use outcomes::{PitchOutcome, BipType, SwingResult, FielderInfo, INFIELD_POS, OUTFIELD_POS, resolve};
...
use render::{View, draw_scene, draw_aim_crosshair, draw_release_fx, draw_release_fx_warning,
             ...
             draw_fielders, draw_hit_fx, draw_hr_fx};
...
const OUTCOME_FX_DURATION: f32 = 1.3;   // strikeout / walk
const HIT_FX_DURATION: f32 = 0.6;
const HR_FX_DURATION: f32 = 2.4;
...
#[derive(Clone, Copy, PartialEq, Debug)]
enum OutcomeFxKind { Strikeout, Walk, Hit, HomeRun }

fn outcome_fx_duration(kind: OutcomeFxKind) -> f32 {
    match kind {
        OutcomeFxKind::Strikeout | OutcomeFxKind::Walk => OUTCOME_FX_DURATION,
        OutcomeFxKind::Hit                              => HIT_FX_DURATION,
        OutcomeFxKind::HomeRun                          => HR_FX_DURATION,
    }
}
...
struct App {
    ...
    // Fielder responsible for the current play (name, honest start x, y).
    // Animates from its honest position toward the ball on BallInPlay.
    active_fielder: Option<FielderInfo>,
...
impl App {
    ...
    fn resolve_pitch(&mut self, t_flight: f32) {
        ...
        self.active_fielder = result.fielder;
                ...
                if let Some(bt) = &r.bip_type {
                    match bt {
                        BipType::Hit(ht) => {
                            ...
                            if fx_enabled {
                                self.outcome_fx = Some((
                                    if is_hr { OutcomeFxKind::HomeRun } else { OutcomeFxKind::Hit },
                                    0.0,
                                ));
                            }
                        }
    ...
    /// For the active fielder animation: an out means they converge on and
    /// "catch" the ball at its first-landing point; a hit means they keep
    /// chasing it all the way to where it finally comes to rest.
    fn fielder_target_and_arrival(&self) -> Option<((f32, f32), f32)> {
        let bip = self.bip.as_ref()?;
        let is_hit = matches!(self.last_result.bip_type, Some(BipType::Hit(_)));
        if is_hit {
            Some(((bip.rest.x, bip.rest.y), bip.duration.max(0.1)))
        } else {
            Some(((bip.land_x, bip.land_y), bip.first_landing_time.max(0.1)))
        }
    }

    /// Current on-screen position of the active fielder, if any — animated
    /// during BallInPlay, held at its arrival point through Result, and not
    /// shown (falls back to the honest static dot) otherwise.
    fn active_fielder_draw_pos(&self) -> Option<FielderInfo> {
        let (name, sx, sy) = self.active_fielder?;
        match self.phase {
            Phase::BallInPlay { t } => {
                if let Some(((tx, ty), arrival)) = self.fielder_target_and_arrival() {
                    let p = (t / arrival).min(1.0);
                    let eased = 1.0 - (1.0 - p) * (1.0 - p);
                    Some((name, sx + (tx - sx) * eased, sy + (ty - sy) * eased))
                } else {
                    Some((name, sx, sy))
                }
            }
            Phase::Result { .. } => {
                if let Some(((tx, ty), _)) = self.fielder_target_and_arrival() {
                    Some((name, tx, ty))
                } else {
                    Some((name, sx, sy))
                }
    ...
    fn update(&mut self) {
        ...
        // Outcome celebration FX ticks independent of phase too.
        if let Some((kind, t)) = self.outcome_fx {
            let dur = outcome_fx_duration(kind);
            let nt = t + raw_dt;
            self.outcome_fx = if nt > dur { None } else { Some((kind, nt)) };
        }
    ...
    fn draw(&mut self, view: &View) {
        ...
        // Defense — 7 fielders (no pitcher/catcher) at their honest
        // positions; the one responsible for the current play animates
        // toward the ball.
        let all_fielders: [(&str, f32, f32); 7] = [
            INFIELD_POS[0], INFIELD_POS[1], INFIELD_POS[2], INFIELD_POS[3],
            OUTFIELD_POS[0], OUTFIELD_POS[1], OUTFIELD_POS[2],
        ];
        draw_fielders(view, &all_fielders, self.active_fielder_draw_pos());
        ...
        // Strikeout (fire) / walk (ice) / hit (lightning) / HR (fireworks) FX
        if let Some((kind, t)) = self.outcome_fx {
            let progress = (t / outcome_fx_duration(kind)).min(1.0);
            match kind {
                OutcomeFxKind::Strikeout => draw_strikeout_fx(progress),
                OutcomeFxKind::Walk      => draw_walk_fx(progress),
                OutcomeFxKind::Hit       => draw_hit_fx(progress),
                OutcomeFxKind::HomeRun   => draw_hr_fx(progress),
            }
        }
    }
...
async fn main() {
    ...
    loop {
        ...
        let view = View::new(&app.data.game, app.role == Role::Pitcher);
