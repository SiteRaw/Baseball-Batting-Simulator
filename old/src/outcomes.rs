...
pub fn resolve(
    ...
    let bip_type = classify_bip(&bip, la, ev, game);
    out.description = describe(&bip_type, &bip, ev, la);
...
// effective range, since there's less time to react.

const INFIELD_POS: [(&str, f32, f32); 4] = [
    ("1B", 48.0, 78.0),
    ("2B", 22.0, 130.0),
    ("SS", -25.0, 135.0),
    ("3B", -48.0, 78.0),
];
const OUTFIELD_POS: [(&str, f32, f32); 3] = [
    ("LF", -110.0, 280.0),
    ("CF", 0.0, 320.0),
    ("RF", 110.0, 280.0),
];
...
fn nearest_coverage(angle_deg: f32, positions: &[(&str, f32, f32)], sigma: f32) -> f32 {
    positions.iter()
        .map(|&(_, fx, fy)| {
...
/// Name of the closest fielder by angle (for flavor text on outs).
fn nearest_position_name<'a>(angle_deg: f32, positions: &[(&'a str, f32, f32)]) -> &'a str {
    positions.iter()
        .map(|&(name, fx, fy)| (name, (angle_deg - fx.atan2(fy).to_degrees()).abs()))
        .min_by(|a, b| a.1.partial_cmp(&b.1).unwrap())
        .map(|(name, _)| name)
        .unwrap_or("?")
}

pub fn classify_bip(bip: &BipFlight, la: f32, ev: f32, game: &GameCfg) -> BipType {
    if bip.is_hr {
        return BipType::Hit(HitType::HomeRun);
    }
    ...
    if la < 8.0 {
        ...
        return if chance(p_hit) { BipType::Hit(HitType::Single) } else { BipType::Groundout };
    }
    ...
    if la < 23.0 {
        ...
        if chance(p_hit) {
            ...
            if chance(p_xbh) { BipType::Hit(HitType::Double) }
            else              { BipType::Hit(HitType::Single) }
        } else {
            BipType::Lineout
        }
    } else {
        ...
        if chance(p_hit) {
            ...
            if chance(xbh) { BipType::Hit(HitType::Double) }
            else            { BipType::Hit(HitType::Single) }
        } else {
            BipType::Flyout
        }
...
fn describe(bt: &BipType, bip: &BipFlight, ev: f32, la: f32) -> String {
    ...
    match bt {
        ...
        BipType::Groundout => {
            let pos = nearest_position_name(angle, &INFIELD_POS);
            format!("Ground out to {pos} ({:.0} mph, {:.0}°)", ev, la)
        }
        BipType::Lineout => {
            let pos = if nearest_coverage(angle, &OUTFIELD_POS, 12.0) > nearest_coverage(angle, &INFIELD_POS, 7.0) {
                nearest_position_name(angle, &OUTFIELD_POS)
            } else {
                nearest_position_name(angle, &INFIELD_POS)
            };
            format!("Line out to {pos} ({:.0} mph, {:.0}°)", ev, la)
        }
        BipType::Flyout => {
            let pos = nearest_position_name(angle, &OUTFIELD_POS);
            format!("Fly out to {pos} ({:.0} ft)", bip.distance)
        }
